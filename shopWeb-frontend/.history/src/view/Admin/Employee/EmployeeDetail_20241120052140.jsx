...
function UserPage({ setAlertMessage,setShowAlert, setType }) {
    ...
    const fetchApiTimeKeeping = async () => {
        console.log(user._id);
        try {
          const res = await axios.get(`http://localhost:3001/api/timekeeping/getByUserId/${user?._id}?sort=createdAt&sort=desc`);   
      ...
      const formatDate = (dateString) => {
        ...
        return date.toLocaleDateString('vi-VN'); // Sử dụng định dạng ngày cho tiếng Việt
      };

      useEffect(() => {
        let totalSalary = 0;
        listTimeKeeping.forEach(item => {
          totalSalary += item.status === "Có mặt" ? 240000 : 0;
          totalSalary -= item.status === "Vắng" ? 120000 : 0;
          totalSalary -= item.status === "Trễ" ? 50000 : 0;
        });
    ...
    return (
        <div style={{height:"auto", marginTop:"130px ",background:"white" }}>
            <div className="user-page">
                ...
                <div className="account-details">
                    ...
                    <div className="account-table">
                        ...
                        <div className="account-table_row">
                            ...
                            {isEditingAddress ? (
                                <>
                                    <AddressForm address={address} setAddress={setAddress} handleSaveAddress={handleSaveAddress} />
                                </>
                            ) : (
                                <>
                ...
                <div className="timekeeping_container employee_timekeeping" style={{width:"65%"}}>
                    <div className="employee_timekeeping-select">
                        <h4>Chấm công </h4>
                        <h4>Lương:<p className="salary"> {salary.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' })}</p> </h4>
                        <input
                        type="date"/>
                    </div>
