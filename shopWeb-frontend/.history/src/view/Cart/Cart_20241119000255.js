  ...
  // Cập nhật giá trị `listCart` khi số lượng sản phẩm thay đổi trong CartItem
    ...
    const handleOrder = async () => {
      ...
      orderProducts.forEach(async (item)=> {
        const countInStock = await axios.get(`http://localhost:3001/api/product/getDetail/${item.productId}`)?.colors.find(
          (color)=>color.color === orderProducts.color
        )
        console.log('countInStock',countInStock,'item',item.amount);
        if(countInStock <= item.amount){
